...
postcode_regex = re.compile(r"^([Gg][Ii][Rr] 0[Aa]{2})|((([A-Za-z][0-9]{1,2})|(([A-Za-z][A-Ha-hJ-Yj-y][0-9]{1,2})|(([AZa-z][0-9][A-Za-z])|([A-Za-z][A-Ha-hJ-Yj-y][0-9]?[A-Za-z])))) [0-9][A-Za-z]{2})$")
...
class ESPC():
	...
	def parse_page(self,page=1):
		...
		status_code,self.data = proxy.get(self.site)
		no_response_text = "Sorry, we found no results which matched your selected criteria. Refine your search."
		if self.data.title == "Server Error" or no_response_text in self.data.text or status_code != 200:
			return None
		house_list=[]
		...
		for house in houses: 
			try:
				...
				full_address=full_address.replace(house_type,"").strip().replace(",","")
				...
				agency = "ESPC"
				row=[
					ts,agency,house_type,full_address,
					postcode,description,price, bathrooms,
					living_rooms,bedrooms, self.area, link
				]
				...
				if check_row(self.filename,row[1:-2]):
					print("Duplicate: row(%s) exists in file:%s"%(row[3],self.filename))
				else:
	...
	def get_areas(self):
		...
		status_code,data = proxy.get(url)
		areas = [
			link for link in data.find_all("a") 
			if link.attrs.get("href","").startswith("/areas/") and  re.match("^\\w*",link.text).group()
		]
		area_dict = dict()
		for area in areas:
			area_name = area.text
			...
			status_code,data = proxy.get(area_url)
			if status_code == 200:
				new_areas = [
					link for link in data.find_all("a") 
					if link.attrs.get("href","").startswith("/areas/") and  re.match("^\\w*",link.text).group()
...
place="scotland"
espc = ESPC(place,proxy,"%s.csv"%place)
espc.parse_areas()
#espc.parse_site()
